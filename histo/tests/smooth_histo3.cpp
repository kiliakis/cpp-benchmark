...
using namespace std;
string prog_name = "smooth_histo3";
long int N_t = 1;
int N_p = 100000;
int N_threads = 1;
int n_slices = 100;
                                 ...
                                 const int n_macroparticles)
{
    ...
    for (int i = 0; i < n_macroparticles; i++) {
        ...
        if (distToCenter > 0.5)
            fffbin = (int)(fbin + 1.0);
        double ratioffbin = 1.5 - distToCenter;
        double ratiofffbin = 1 - ratioffbin;
        if (distToCenter < 0.5)
            fffbin = (int)(fbin - 1.0);
...
int main(int argc, char *argv[])
{
    ...
    double other_d = elapsed_time.count() - set_zero_d.count() - histo_calc_d.count();
